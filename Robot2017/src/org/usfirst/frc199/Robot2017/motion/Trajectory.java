...
public class Trajectory {
	 ...
	 */
	public Trajectory(Path path, double v0, double v1, double vmax, double amax, double wmax, double alphamax, double jerkmax,
			int points) {
		this.path = path;
		this.vmax = vmax;
		this.amax = amax;
	...
	private void computeForwardTrajectory() {
		for (int i = 1; i < velocities.length - 1; i++) {
			...
			double amax = Math.min(getAmax(i, i + 1, vprev), aprev + jerkmax);
//			if (vprev + amax < vmax) {
...
//			}
			...
			velocities[i] = Math.min(vprev + amax, vmax);
		}
	}
