...
/**
 * A path defined by parametric cubic functions of a parameter s. s is a spline
 * that represents progress along the path as a value from 0 to 1.
 */
public class Path {
	...
	public Path(double x0, double y0, double x1, double y1, double angle0, double angle1, double k) {
		...
		double vy0 = Math.cos(Math.toRadians(angle0)) * k * Math.sqrt(dx * dx + dy * dy); // Sam
																							// was
																							// here.
	...
	/**
	 * Gets angular acceleration with respect to arc length at a point on the
	 * path
	 * 
	 * @param s - the point on the path from 0 to 1
	...
	/**
	 * Gets the s value for a given arc length Stores previous value to improve
	 * efficiency
	 * 
	 * @param L - arc length
